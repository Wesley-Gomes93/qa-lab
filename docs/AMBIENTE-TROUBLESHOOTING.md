...
Isso sobe na ordem: **PostgreSQL → Backend → Frontend**.
...
### 2. "Port already in use" (3000, 4000, 5432)

**Causa:** Porta ocupada por outro processo ou instância anterior.

**Solução:**
```bash
# Ver o que está na porta (ex: 3000)
lsof -i :3000
...
**Solução:** Sempre rode `make dev` ou `npm run dev` **na raiz do projeto**, não dentro de `tests/`, `frontend/`, etc.
